...
env.Install('$JS_DISTDIR', 'samples')
if env['OS_CONF'] == 'windows':
    env.Install('$JS_DISTDIR/plugin', ['alljoyn64.reg', 'alljoyn.reg', '$JS_DISTDIR/lib/npalljoyn.dll'])
elif env['OS_CONF'] == 'android':
    env.Install('$JS_DISTDIR/plugin', ['AndroidManifest.xml', '.classpath', 'default.properties', '.project', 'res', 'src'])
    env.Install('$JS_DISTDIR/plugin/libs/armeabi', '$JS_DISTDIR/lib/libnpalljoyn.so')
