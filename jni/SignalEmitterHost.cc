...
_SignalEmitterHost::_SignalEmitterHost(Plugin& plugin, BusObject& busObject, const ajn::InterfaceDescription::Member* signal)
...
bool _SignalEmitterHost::emitSignal(const NPVariant* npargs, uint32_t npargCount, NPVariant* npresult)
{
    ...
    size_t numArgs = ajn::SignatureUtils::CountCompleteTypes(signal->signature.c_str());
    ...
    if (numArgs < npargCount) {
        NPVariant params = npargs[npargCount - 1];
        if (!NPVARIANT_IS_OBJECT(params)) {
            typeError = true;
            char message[128];
            snprintf(message, sizeof(message), "argument %d is not an object", npargCount - 1);
            plugin->RaiseTypeError(message);
            goto exit;
        }
