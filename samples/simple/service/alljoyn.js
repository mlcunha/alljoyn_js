...
var service = (function() {
        ...
        var start = function() {
            ...
            if (status) {
                alert("Bind session port failed [(" + status + ")]");
                return;
            }
            /* Request a well-known name from the bus... */
            status = bus.requestName(SERVICE_NAME, aj.BusAttachment.DBUS_NAME_FLAG_REPLACE_EXISTING | 
                                                   aj.BusAttachment.DBUS_NAME_FLAG_DO_NOT_QUEUE);
            if (status) {
                alert("Request '" + SERVICE_NAME + "' failed [(" + status + ")]");
                return;
            }
            /* ...and advertise the same well-known name. */
            status = bus.advertiseName(SERVICE_NAME, aj.SessionOpts.TRANSPORT_ANY);
            if (status) {
        ...
        that.stop = function() {
            ...
            /* 
             * Unregister the bus object.  'delete bus["/SimpleService"]' does not work in all
             * browsers. 
             */
            bus["/SimpleService"] = undefined;
